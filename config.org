...
Let's use escape to exit from anywhere similar to using `C-g`.
The code below is also safe to use with evil, as it does not break evil bindings.
#+begin_src emacs-lisp
...
**** Projectile
***** Bindings
The default projectile key bindings changed recently, but I'm used to the old ones.
So, let's set them according to my tastes.
#+begin_src emacs-lisp
...
#+RESULTS:
: my-dired-org-link-to-clipboard
...
**** Template
  ...
  #+BEGIN_SRC conf :tangle ~/Documents/org/templates/weight.orgtmpl
...
(use-package! copilot
  :config
  (advice-add 'company-indent-or-complete-common :before #'copilot-accept-completion)
