...
(after! mu4e
  ...
  ;; This is probably causing issues, so let mbsync do the work
...
;; Key bindings
;;
(map! :map mu4e-view-mode-map :n "C-<tab>" #'mu4e-view-headers-next-unread)
(map! :map mu4e-headers-mode-map :n "C-<tab>" #'mu4e-headers-next-unread)
